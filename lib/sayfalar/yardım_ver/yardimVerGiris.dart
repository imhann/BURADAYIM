...
import 'package:buradayim/sayfalar/yard%C4%B1m_ver/bagislarim.dart';
import 'package:buradayim/sayfalar/yard%C4%B1m_ver/listeleme.dart';
...
class YardimVerGiris extends StatefulWidget {
  KisiModel kisiModel;
  ...
  @override
  State<YardimVerGiris> createState() => _YardimVerGirisState();
}

class _YardimVerGirisState extends State<YardimVerGiris> {
  //Veri tabanını çağır
...
//Konum bilgisini aldığımız alan
          ...
          child: TextButton(
              onPressed: () {
                Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => BagislarimSayfasi(
                    kisiModel: widget.kisiModel,
                  ),
                ));
              },
              child: const Text('BAĞIŞLARIM'))),
    );
  }
  ...
  showAlertDialog(BuildContext context) {
    ...
    AlertDialog alert = AlertDialog(
      ...
      content: const Text("BAĞIŞLARINIZ ALINMIŞTIR TEŞEKKÜR EDERİZ..."),
      actions: [
        ElevatedButton(
          child: const Text('Bağış oluştur'),
          onPressed: () async {
            ...
            if (listeUrun!.isNotEmpty) {
              ...
              Navigator.of(context).pushAndRemoveUntil(
                  MaterialPageRoute(
                    builder: (context) => YardimVerGiris(
                      kisiModel: widget.kisiModel,
  ...
  void urunMiktariniAyarlama() {
    for (var element in listeUrun!) {
      for (var i = 0; i < allUrunler.length; i++) {
        if (element.urunAdi == allUrunler[i].urunAdi) {
          var guncelle = Urunler(
              yardimAlanmi: false,
              urunAdi: allUrunler[i].urunAdi,
              urunAdet: allUrunler[i].urunAdet + element.girilenUrunMiktari,
              girilenUrunMiktari: 0);
